...
window.prefetch = async (path) => {
  ...
  files.forEach(async file => {
    if(file.endsWith(".css")){
      ...
      await fetch.open('GET', file, false);
      ...
      const transpile = await Babel.transform(source, { presets: ['es2015', 'react'], filename: 'app.jsx' }).code;
...
console.log(`%cPrefetched ${file} | ${Math.round(size)} ${sizevar[i]}`, 'color: violet; font-weight: bold; font-size: 1.2em;')        
      ...
      const source = fetch.responseText;
      const componentName = file.split('/').pop().split('.jsx')[0];
      const transpile = await Babel.transform(source, { presets: ['es2015','typescript' ,'react'], filename: 'app.tsx' }).code;
      cache[file] = transpile;
      let size = transpile.length
      let sizevar = ["B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"]
      let i = 0
      ...
      let transpile = await Babel.transform(source, { presets: ['es2015','typescript'], filename: 'app.ts' }).code;
      cache[file] = transpile;
