import   'https://unpkg.com/vue@3.3.2/dist/vue.global.js';
class Lazy {
...
let root = null;
 ...
 window.visiVersion = "1.0.0-vue-beta"
 // get the latest version with -vue tag

 fetch('https://registry.npmjs.org/visi.js').then(res => res.json()).then(json => {
    let version = json['dist-tags'].latest
    if(version.includes("-vue") &&  window.visiVersion !== version){
        console.assert(false, `You are using an outdated version of visi.js. Please update to ${version} by running npm i visi.js@latest`,{
            version: window.visiVersion,
            latest: version
        })
    }
  ...
  const dispose = async (path, callback, props = {}) => {
    ...
    if (path.endsWith('.vue')) {
      fetch(path)
        .then((response) => response.text())
        .then((code) => {
          // Compile the Vue component using Vue.js Compiler or Vueify
            const component = Vue.compile(code);
             console.log(component)

        })
    } else {
       throw new Error("Only .vue files are supported")
    }
  };
